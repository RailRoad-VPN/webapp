...
class PayProGlobalPaymentService(object):
    ...
    def build_redirect_url(self, user_uuid: UUID, order_code: str, subscription_id: int, payment_method_id: str,
        ...
        if subscription_uuid is not None and isinstance(subscription_uuid, str):
            subscription_uuid = uuid.UUID(subscription_uuid)

        payment_method_id = str(payment_method_id)
        self.logger.info('Buy or Recurring? Depends on payment method.')
        if payment_method_id in ['1', '14']:
            self.logger.info('Payment method is CC or PayPal. Use RECURRING payments')
            ...
            self.logger.info('Payment method is NOT CC or PayPal. Use BUY payments')
        ...
        self.logger.info('Production ID: %s' % product_id)

        self.logger.info('Create base url with base required parameters')
        redirect_url = self._base_url % (str(product_id), str(self._test_mode), str(self._secret_key))
        self.logger.info('Phrase #1. Redirect URL: %s' % str(redirect_url))

        self.logger.info('Add payment and order fields to URL.')
        ...
        redirect_url += self._params_dict['user_uuid'] % user_uuid.hex
        if subscription_uuid is not None:
            redirect_url += self._params_dict['subscription_uuid'] % subscription_uuid.hex

        self.logger.info('Phrase #1. Redirect URL: %s' % str(redirect_url))

        self.logger.info('Check user locale')
        if user_locale:
            self.logger.info('We have user locale: %s' % str(user_locale))
            if len(user_locale) > 2:
                self.logger.info('Too long user locale. Make it short.')
                user_locale = user_locale[0:2]
                self.logger.info('New user locale: %s' % str(user_locale))
            redirect_url += self._params_dict['language'] % str(user_locale)
            self.logger.info('Phrase #2. Redirect URL: %s' % str(redirect_url))
...
class SubscriptionService(object):
    ...
    def get_subscriptions(self, lang_code) -> list:
        ...
        if subscriptions is None:
            self.logger.info("There are no subscriptions in cache. Call billing service")
    ...
    def get_subscriptions_dict(self, lang_code) -> dict:
        self.logger.info("Check subscriptions_dict in cache")
        subscriptions_dict = self._cache_service.get(key='subscriptions_dict', prefix=lang_code)
        self.logger.info("Is subscriptions dict exists in cache: %s" % subscriptions_dict is not None)
        if subscriptions_dict is None:
            self.logger.info("There are no subscriptions_dict in cache.")
            self.__update_cache_subscriptions(lang_code=lang_code)
